...
*/
/*
 * C test cases for the module iop/filmicrgb.c, called by the Bazel GTest
 * runner in filmicrgb_gtest.cc.
 */
#include <limits.h>
#include <stddef.h>
#include <stdio.h>
...
#define assert_int_equal(actual, expected) assert((actual) == (expected))
#define assert_string_equal(actual, expected) assert(strcmp((actual), (expected)) == 0)
#define assert_float_equal(actual, expected, epsilon) assert(fabsf((actual) - (expected)) <= (epsilon))
